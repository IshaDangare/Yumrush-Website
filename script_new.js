...
const RESTAURANT = {
    name: 'YumRush Food',
    location: 'Malad East, Tanaji Nagar, Near Times of India Gate',
    phone: '+91-9136535275',
    instagram: '@yumrushfood',
    coordinates: { lat: 19.1886, lng: 72.8349 },
...
function initializeMap() {
    ...
    // Draw delivery area circle (5km)
    const deliveryCircle = new google.maps.Circle({
        center: restaurantLocation,
        radius: RESTAURANT.maxDeliveryKm * 1000, // Convert KM to meters
        map: map,
        fillColor: '#ff7a00',
        fillOpacity: 0.1,
        strokeColor: '#ff7a00',
...
function calculateTimings() {
    ...
    // Dispatch time: current time + 15 mins + (2 mins per KM)
    const dispatchMinutes = 15 + (selectedDistance * 2);
    const dispatchTime = new Date(now.getTime() + dispatchMinutes * 60000);
    const dispatchTimeStr = dispatchTime.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' });
    
    // Delivery time: dispatch time + (3 mins per KM)
    const deliveryMinutes = dispatchMinutes + (selectedDistance * 3);
    ...
    if (dispatchTimeEl) dispatchTimeEl.textContent = dispatchTimeStr + ' (~' + Math.round(dispatchMinutes) + ' mins)';
    if (deliveryTimeEl) deliveryTimeEl.textContent = deliveryTimeStr + ' (~' + Math.round(deliveryMinutes) + ' mins)';
}
...
function generatePaymentSlip() {
    ...
    if (!name || !phone || !address || selectedDistance === 0) {
...
function order() {
    ...
    msg += `📍 Location: ${selectedLocationName}%0A%0A`;
    ...
    msg += `Estimated Dispatch: ${dispatchTimeStr} (~${Math.round(dispatchMinutes)} mins)%0A`;
    msg += `Estimated Delivery: ${deliveryTimeStr} (~${Math.round(deliveryMinutes)} mins)%0A%0A`;
